...
describe('List', () => {
  ...
  describe('tail()', () => {
    ...
    const tests: {
      ...
      {
        name: 'should return the empty list when there is only one element',
        fields: {list: [42]},
        want: new List([]),
      },
      {
        name: 'should return the list without the first element',
        fields: {list: [42, 56, 78]},
        want: new List([56, 78]),
      },
    ];
    ...
    tests.forEach((tt) => {
      it(tt.name, () => {
        const list = new List(tt.fields.list);
        expect(list.tail()).toEqual(tt.want);
      });
    });
  });
  ...
  describe('some()', () => {
    ...
    const tests: {
      ...
      {
        name:
          'passing isEven should return true when at least one number is even',
  ...
  describe('every()', () => {
    ...
    const tests: {
      ...
      {
        name:
          'passing isEven should return false when only some number are even',
        fields: {list: [3, 93, 42, 23, 45]},
        args: {predicate: (n: number) => n % 2 === 0},
        want: false,
      },
